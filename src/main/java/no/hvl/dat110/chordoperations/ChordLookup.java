...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for(int i = fingerTable.size()-1; i>=0;i--){
			...
			boolean isInInterval = Util.checkInterval(fingerId,node.getNodeID().add(BigInteger.ONE),ID.subtract(BigInteger.ONE));
			if(isInInterval){
				return finger;
			}
		}
		...
		// In the case that no finger is the closest predecessor, we need to return the current node
