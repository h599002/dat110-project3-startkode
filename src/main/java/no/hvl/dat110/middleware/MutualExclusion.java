...
public class MutualExclusion {
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for(Message m : activenodes){
			...
			Util.getProcessStub(m.getNodeName(),m.getPort());
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 0: {
				...
				NodeInterface node1 = Util.getProcessStub(message.getNodeName(),message.getPort());
				// acknowledge message
				message.setAcknowledged(true);
				// send acknowledgement back by calling onMutexAcknowledgementReceived()
				assert node1 != null;
				node1.onMutexAcknowledgementReceived(message);
				break;
			}
			...
			case 2: {
				...
				BigInteger receiverNodeID = this.node.getNodeID();
				// compare clocks, the lowest wins
				// Sender wins means that senderclock is smallest or its a tie and the ID is lower
				boolean senderWins = (senderClock < receiverClock) || (senderClock == receiverClock && (senderNodeID.compareTo(receiverNodeID) < 0) );
				if(senderWins){
					message.setAcknowledged(true);
